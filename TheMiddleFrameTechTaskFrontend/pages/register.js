...
export default function Register() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col justify-center py-12 px-6 lg:px-8 selection:bg-indigo-100">
      <Head>
        <title>Join Us | TMF Marketplace</title>
      </Head>
      <div className="sm:mx-auto sm:w-full sm:max-w-md">
        ...
        <p className="mt-2 text-center text-sm text-slate-500">
          Join our curated photography network
        </p>
      </div>
      <div className="mt-10 sm:mx-auto sm:w-full sm:max-w-md">
        <div className="bg-white py-12 px-10 shadow-xl shadow-slate-200/50 rounded-[2.5rem] border border-slate-100">
          ...
          <div className="flex p-1 bg-slate-100 rounded-xl mb-10 border border-slate-200">
            <button
              onClick={() => setRole('buyer')}
              className={`flex-1 py-3 text-xs font-black uppercase tracking-widest rounded-lg transition-all ${role === 'buyer' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}
            >
              Buyer
            </button>
            <button
              onClick={() => setRole('photographer')}
              className={`flex-1 py-3 text-xs font-black uppercase tracking-widest rounded-lg transition-all ${role === 'photographer' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}
            >
              Uploader
            </button>
          </div>
          <form className="space-y-6" action="#" method="POST">
            <div>
              <label htmlFor="name" className="block text-[11px] font-black text-slate-400 uppercase tracking-[0.2em] ml-1 mb-2">
                Full display name
              </label>
              <div className="mt-1">
                <input
            ...
            <div>
              <label htmlFor="password" className="block text-[11px] font-black text-slate-400 uppercase tracking-[0.2em] ml-1 mb-2">
                Secure password
              </label>
              <div className="mt-1">
                <input
                ...
                onClick={async (e) => {
                  ...
                  try {
                    ...
                    if (res.success) {
                      ...
                      window.location.href = `/?role=${res.user.role}`;
                    } else {
          ...
          <div className="mt-10 pt-10 border-t border-slate-50">
            <p className="text-center text-xs font-bold text-slate-400 uppercase tracking-widest">
              Already exploring?{' '}
              <Link href="/login">
                <a className="text-indigo-600 hover:text-indigo-500 transition-colors ml-1">Sign in here</a>
              </Link>
            </p>
          </div>
        </div>
